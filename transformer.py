...
class TransformerModel(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, output_dim):
        ...
        self.positional_encoding = nn.Parameter(torch.zeros(1, 1000, d_model))  # Adjust the size as necessary
...
model = TransformerModel(input_dim=input_dim, d_model=d_model, nhead=nhead, num_layers=num_layers, output_dim=output_dim).to(device)
...
plt.legend()
plt.savefig("transformer-1.png", dpi=1080)  # 保存图片，分辨率为600
plt.show()
...
plt.savefig("transformer_residuals.png", dpi=1080)  # 保存图片，分辨率为1080
plt.show()
...
plt.legend()
plt.savefig("transformer-3.png", dpi=1080)  # 保存图片，分辨率为600
plt.show()